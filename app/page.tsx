  ...
  console.info('login response', data);
      if (data.success) {
        ...
        if ((userType === "admin" && data.user_id.startsWith("01")) || (userType === "technician" && data.user_id.startsWith("02"))) {
          showNotification("success", `เข้าสู่ระบบสำเร็จ ยินดีต้อนรับ ${data.name}`);
          // save user info to localStorage
          localStorage.setItem("user", JSON.stringify(data));
          setTimeout(() => {
            if (data.type_id === "01") {
              window.location.href = "/admin/dashboard";
            } else if (data.type_id === "02") {
                ...
                <div className="space-y-2 text-xs bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-blue-600">Admin:</span>
                     <span className="font-mono bg-white px-2 py-1 rounded">admin / 1234</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-orange-600">Technician:</span>
                     <span className="font-mono bg-white px-2 py-1 rounded">tech1 / 2345</span>
                  </div>
                </div>
              </div>
            </div>
