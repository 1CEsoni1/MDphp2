...
export async function PATCH(req: Request, { params }: any) {
		try {
			...
			const { status, assignedTo, notes, changedBy, files } = body;
				...
				// Get current status
				const [rows]: any = await conn.execute('SELECT id, status FROM tb_repair_requests WHERE id = ?', [ctxParams.id]);
				if (rows.length === 0) {
						await conn.end();
		...
		const oldStatusValue = oldStatus ?? '';
				...
				await conn.execute(
						...
						[status, assignedTo || null, notes || null, ctxParams.id]
				);
				...
				if (Array.isArray(files) && files.length > 0) {
					...
					for (const f of files) {
						try {
							...
							// store filename only to avoid large packet
					...
					if (savedFilesObjects.length > 0) {
						...
						if (changedBy) {
							try {
								for (const fn of savedFilenames) {
									await conn.execute('INSERT INTO tb_images (repair_request_id, image_url, uploaded_by) VALUES (?, ?, ?)', [ctxParams.id, fn, changedBy]);
								}
							} catch (insErr) {
						...
						// Always try to update images column (store JSON blob of objects)
						try {
							...
							try {
								await conn.execute('UPDATE tb_repair_requests SET images = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [JSON.stringify(merged), ctxParams.id]);
								imagesUpdated = true;
							} catch (updErr) {
